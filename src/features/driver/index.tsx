      ...
      <header className="bg-white border-b border-gray-200 px-4 py-4 sticky top-0 z-10 pt-safe">
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <p className="text-base text-gray-500 mt-0.5">{today}</p>
          </div>
          <button
            onClick={signOut}
            className="shrink-0 min-h-12 min-w-12 px-3 py-3 text-base font-medium text-gray-600 active:text-gray-900 active:bg-gray-100 rounded-xl"
          >
            Sign out
          </button>
        </div>
          ...
          jobs.map(j => {
            ...
            return (
              <button
                ...
                className={`w-full text-left bg-white rounded-2xl border border-gray-200 p-5 min-h-[5.5rem] active:bg-gray-50 transition-colors ${done ? 'opacity-60' : ''}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    {j.site_address && (
                      <p className="text-base text-gray-600 mt-1 truncate">{j.site_address}</p>
                    )}
